...
namespace vk {
    ...
    VkImageView makeImageView(Context context, VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipLevels) {
        ...
        VkImageView view;
        if (vkCreateImageView(context.device, &info, nullptr, &view) != VK_SUCCESS) {
...
public:
    void setupImGui() {
        ...
        QueueFamilyIndices indices = findQueueFamilies(mContext.physicalDevice);
        ...
        VkResult err = vkCreateDescriptorPool(mContext.device, &pool_info, nullptr, &imguiDescriptorPool);
        check_vk_result(err);
...
private:
    ...
    void initVulkan() {
        ...
        textureImageView = vk::makeImageView(mContext, textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT, mipLevels);
        textureSampler = vk::makeSampler(mContext, mipLevels);
    ...
    void cleanupSwapChain() {
        vkDestroyImageView(mContext.device, depthImageView, nullptr);
        vkDestroyImage(mContext.device, depthImage, nullptr);
        vkFreeMemory(mContext.device, depthImageMemory, nullptr);
    ...
    void cleanup() {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            vkDestroyBuffer(mContext.device, uniformBuffers[i], nullptr);
            vkFreeMemory(mContext.device, uniformBuffersMemory[i], nullptr);
        }

        vkDestroyDescriptorPool(mContext.device, imguiDescriptorPool, nullptr);
        vkDestroyDescriptorPool(mContext.device, descriptorPool, nullptr);

        vkDestroySampler(mContext.device, textureSampler, nullptr);
        vkDestroyImageView(mContext.device, textureImageView, nullptr);

        vkDestroyImage(mContext.device, textureImage, nullptr);
        vkFreeMemory(mContext.device, textureImageMemory, nullptr);

        vkDestroyDescriptorSetLayout(mContext.device, descriptorSetLayout, nullptr);

        mesh.clean(mContext.device);

        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            vkDestroySemaphore(mContext.device, renderFinishedSemaphores[i], nullptr);
            vkDestroySemaphore(mContext.device, imageAvailableSemaphores[i], nullptr);
            vkDestroyFence(mContext.device, inFlightFences[i], nullptr);
        }
    ...
    void pickPhysicalDevice() {
        ...
        for (const auto& device : devices) {
            if (isDeviceSuitable(device)) {
                mContext.physicalDevice = device;
                break;
            }
        }

        if (mContext.physicalDevice == VK_NULL_HANDLE) {
            throw std::runtime_error("failed to find a suitable GPU!");
        }

        if (!gladLoaderLoadVulkan(instance, mContext.physicalDevice, nullptr)) {
    ...
    void createLogicalDevice() {
        ...
        if (vkCreateDevice(mContext.physicalDevice, &createInfo, nullptr, &mContext.device) != VK_SUCCESS) {
            throw std::runtime_error("failed to create logical device!");
        }

        vkGetDeviceQueue(mContext.device, indices.graphicsFamily.value(), 0, &graphicsQueue);
        vkGetDeviceQueue(mContext.device, indices.presentFamily.value(), 0, &presentQueue);

        if (!gladLoaderLoadVulkan(instance, mContext.physicalDevice, mContext.device)) {
            throw std::runtime_error("failed to reload vulkan functions");
        }
    }

    void createSwapChain() {
        SwapChainSupportDetails swapChainSupport = querySwapChainSupport(mContext.physicalDevice);
    ...
    void createImageViews() {
        ...
        for (uint32_t i = 0; i < swapChainImages.size(); i++) {
            swapChainImageViews[i] = vk::makeImageView(mContext, swapChainImages[i], swapChainImageFormat, VK_IMAGE_ASPECT_COLOR_BIT, 1);
        }
    }
    ...
    void createDescriptorSetLayout() {
        ...
        if (vkCreateDescriptorSetLayout(mContext.device, &layoutInfo, nullptr, &descriptorSetLayout) != VK_SUCCESS) {
    ...
    void createFramebuffers() {
        ...
        for (size_t i = 0; i < swapChainImageViews.size(); i++) {
            ...
            if (vkCreateFramebuffer(mContext.device, &framebufferInfo, nullptr, &swapChainFramebuffers[i]) != VK_SUCCESS) {
    ...
    void createCommandPool() {
        QueueFamilyIndices queueFamilyIndices = findQueueFamilies(mContext.physicalDevice);
        ...
        if (vkCreateCommandPool(mContext.device, &poolInfo, nullptr, &commandPool) != VK_SUCCESS) {
    ...
    void createDepthResources() {
        ...
        depthImageView = vk::makeImageView(mContext, depthImage, depthFormat, VK_IMAGE_ASPECT_DEPTH_BIT, 1);
    }
    ...
    void createTextureImage(char const* filepath) {
        ...
        void* data;
        vkMapMemory(mContext.device, stagingBufferMemory, 0, imageSize, 0, &data);
        memcpy(data, image.pixels(), static_cast<size_t>(imageSize));
        vkUnmapMemory(mContext.device, stagingBufferMemory);

        createImage(image.width(), image.height(), mipLevels, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, textureImage, textureImageMemory);
    ...
    void createMeshBuffer(std::span<const std::byte> bytes, VkBufferUsageFlags usage, VkBuffer& targetBuffer, VkDeviceMemory& targetMemory) {
        ...
        createBuffer(bufferSize, usage | VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, targetBuffer, targetMemory);
    ...
    void createUniformBuffers() {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            ...
            vkMapMemory(mContext.device, uniformBuffersMemory[i], 0, bufferSize, 0, &uniformBuffersMapped[i]);
        }
    }
    ...
    void createDescriptorPool() {
        ...
        if (vkCreateDescriptorPool(mContext.device, &poolInfo, nullptr, &descriptorPool) != VK_SUCCESS) {
    ...
    void createDescriptorSets() {
        ...
        if (vkAllocateDescriptorSets(mContext.device, &allocInfo, descriptorSets.data()) != VK_SUCCESS) {
    ...
    void createSyncObjects() {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            if (vkCreateSemaphore(mContext.device, &semaphoreInfo, nullptr, &imageAvailableSemaphores[i]) != VK_SUCCESS ||
                vkCreateSemaphore(mContext.device, &semaphoreInfo, nullptr, &renderFinishedSemaphores[i]) != VK_SUCCESS ||
                vkCreateFence(mContext.device, &fenceInfo, nullptr, &inFlightFences[i]) != VK_SUCCESS) {
    ...
    void drawFrame() {
        ...
        uint32_t imageIndex;
        VkResult result = vkAcquireNextImageKHR(mContext.device, swapChain, UINT64_MAX, imageAvailableSemaphores[currentFrame], VK_NULL_HANDLE, &imageIndex);
        ...
        vkResetFences(mContext.device, 1, &inFlightFences[currentFrame]);
